...
echo "   2. sudo tailscale up --authkey tskey-auth-xxxx...   # Key: login.tailscale.com → Settings → Keys"
echo "   3. cd $LAB_DIR && cp .env.example .env && nano .env"
echo "      → Especially: GHCR_TOKEN (read:packages PAT from github.com/settings/tokens)"
